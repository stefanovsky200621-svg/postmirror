...
При проверке сценария вручную: пустая форма не открывает окно; смена размера/доставки меняет сумму; при самовывозе адрес скрыт и отсутствует в заявке; для курьера по СПб и доставки по России адрес обязателен; ввод «бала» предлагает Балаково и Балашов; в тексте присутствуют все поля без отдельного контактного Instagram; копирование работает или даёт понятную инструкцию; закрытие/повторное оформление сохраняет актуальные данные; кнопки ведут на нужные аккаунты. Тестовые сообщения владельцу отправлять не нужно.
