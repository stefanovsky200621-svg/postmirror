...
- Обязательные поля: ник/пост для зеркала, ФИО и телефон. Поля Telegram и Instagram клиента удалены: покупатель отправляет готовую заявку со своего аккаунта, поэтому отдельный ник для связи не запрашивается. Поле `instagramTag` относится только к посту, который нужно воплотить в зеркале. Адрес обязателен для доставки и необязателен при самовывозе. Для «Свой размер» появляется обязательное поле `customSize` с желаемыми габаритами, которые включаются в текст заявки. Комментарий необязателен.
- Поле адреса связано с локальным `datalist` из массива `citySuggestions`: при вводе города браузер показывает варианты, включая Балаково и Балашов. После выбора города улицу, дом и квартиру дописывают вручную. Любой адрес можно ввести без подсказки. Внешний адресный сервис не используется, поэтому улицы и дома автоматически не подбираются и введённый адрес никуда не отправляется до действий самого покупателя.
...
При проверке сценария вручную: пустая форма не открывает окно; смена размера/доставки меняет сумму; адрес обязателен только для доставки; ввод «бала» предлагает Балаково и Балашов; в тексте присутствуют все поля без отдельного контактного Instagram; копирование работает или даёт понятную инструкцию; закрытие/повторное оформление сохраняет актуальные данные; кнопки ведут на нужные аккаунты. Тестовые сообщения владельцу отправлять не нужно.
