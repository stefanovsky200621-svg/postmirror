...
- Обязательные поля: ник/пост, ФИО, Telegram, телефон. Адрес обязателен для доставки и необязателен при самовывозе. Для «Свой размер» появляется обязательное поле `customSize` с желаемыми габаритами, которые включаются в текст заявки. Комментарий необязателен.
