...
- Обязательные поля: ник/пост для зеркала, ФИО, Instagram для связи и телефон. Поле Telegram клиента удалено по решению владельца. `instagramContact` выводится в сообщении отдельной строкой «Instagram:». Поле `instagramTag` относится к оформлению зеркала и не заменяет контактный Instagram. Адрес обязателен для доставки и необязателен при самовывозе. Для «Свой размер» появляется обязательное поле `customSize` с желаемыми габаритами, которые включаются в текст заявки. Комментарий необязателен.
