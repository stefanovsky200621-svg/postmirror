...
Актуально на 23 сентября 2026 года. Обновляйте эту карту при изменении структуры, контактов, условий или сценария заказа.

## Назначение

Одна страница с формой заказа поверх полноэкранного фото зеркала без текста. Покупатель заполняет данные и нажимает «Оформить заказ». Открывается компактное окно с текстом заявки, отдельной кнопкой копирования и кнопками Telegram / Instagram. Можно отправить текст или скриншот. Покупатель отправляет сообщение самостоятельно, а бренд уточняет стоимость и подтверждает заказ в переписке.
...
| Доступные размеры зеркал | `app/page.tsx`, массив `sizes` |
| Виды доставки | `app/page.tsx`, массив `deliveries` |
...
- Размер и доставка хранятся в состоянии React и включаются в заявку. Цены на зеркала и доставку на сайте не показываются и не рассчитываются: стоимость уточняется при подтверждении заказа в переписке.
...
- Окно показывает снимок данных на момент оформления. После возврата к форме и повторного оформления текст собирается заново. Ценовых строк в сообщении нет. Размер и способ доставки сохраняются среди параметров заказа. Пустые адрес и комментарий не добавляются, приветствие и заключительная просьба убраны для компактности.
...
В форме оставлены два варианта размера. Остальные размеры из первой версии удалены из выбора.
...
Доставка: самовывоз, курьер по СПб или доставка по России. Место самовывоза и стоимость каждого варианта уточняются в переписке. Публичных цен и автоматического расчёта на сайте нет.
...
Сборка, TypeScript и проверка изменённых файлов прошли после последней переделки. На опубликованной версии проверена цепочка подсказок «Саратовская область → Балаково → улица Ленина → дом 15» и компактная сборка выбранного адреса в тексте заказа. Ранее форма, фотофон, окно заказа и копирование проверялись на ширинах 1440, 390 и 360 px. Реальные iOS/Android-устройства не тестировались.
...
При проверке сценария вручную: пустая форма не открывает окно; смена размера/доставки меняет параметры заявки без показа цен; при самовывозе весь адрес скрыт и отсутствует в заявке; для курьера по СПб и доставки по России область, город/район, улица и дом выбираются из подсказок и обязательны; посёлок и квартира необязательны; смена области очищает все уровни ниже; части адреса объединяются в одну строку заявки от большего к меньшему; копирование работает или даёт понятную инструкцию; закрытие/повторное оформление сохраняет актуальные данные; кнопки ведут на нужные аккаунты. Тестовые сообщения владельцу отправлять не нужно.
...
После изменения функции заказа обновляйте раздел «Сценарий и данные». После изменения полей, условий, фотографий или контактов обновляйте соответствующие разделы этой карты. После изменения доступа или адреса обновляйте раздел «Размещение». Так карта остаётся полезной точкой входа, а не описанием устаревшей версии.
