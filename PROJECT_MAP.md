...
- Обязательные поля: ник/пост для зеркала, ФИО, телефон и хотя бы один аккаунт для связи. Контакты разделены на `telegram` («Telegram для связи») и `instagramContact` («Instagram для связи»); состояния `telegramContact` и `instagramContact` задают взаимную обязательность. Можно заполнить один или оба. В сообщении каждый заполненный контакт идёт отдельной строкой «Telegram:» или «Instagram:», пустые строки не добавляются. Поле `instagramTag` относится к оформлению зеркала и не заменяет контактный Instagram. Адрес обязателен для доставки и необязателен при самовывозе. Для «Свой размер» появляется обязательное поле `customSize` с желаемыми габаритами, которые включаются в текст заявки. Комментарий необязателен.
...
- На компьютере форма находится справа, зеркало видно слева. При ширине до 760 px форма занимает доступную ширину с небольшим открытым участком фонового фото сверху. При ширине до 430 px контактные поля стоят в один столбец.
...
Сборка и TypeScript прошли после последней переделки. Локальная собранная страница работает. Проведена браузерная проверка на ширинах 1440, 390 и 360 px: форма, новый фотофон, открытие заказа и успешное копирование. Типичный заказ с ФИО, адресом и контактами вместе с обеими соцсетями помещается в окно 390 × 844 и 360 × 740 px. Реальные iOS/Android-устройства не тестировались.
