...
Актуально на 9 сентября 2026 года. Обновляйте эту карту при изменении структуры, контактов, цен или сценария заказа.

## Назначение

Одна страница с формой заказа зеркала и примерами оформления. Покупатель заполняет данные, видит предварительную стоимость и нажимает «Оформить заказ». Открывается окно с текстом заявки, отдельной кнопкой копирования и кнопками Telegram / Instagram. Покупатель отправляет сообщение самостоятельно, а бренд подтверждает заказ в переписке.
...
public/
  postmirror-example-1.png  Основной пример из предоставленного чата
  postmirror-example-2.png  Второй пример из предоставленного чата
...
- Обязательные поля: ник/пост для зеркала, ФИО, Telegram / Instagram для связи, телефон. Контактное поле сохраняет техническое имя `telegram`, но принимает ник любой из двух соцсетей; в форме и тексте заказа явно указано его назначение для связи. Адрес обязателен для доставки и необязателен при самовывозе. Для «Свой размер» появляется обязательное поле `customSize` с желаемыми габаритами, которые включаются в текст заявки. Комментарий необязателен.
...
- Серверного приёма заказов, базы данных, Telegram-бота, оплаты, localStorage и автоматической отправки сообщений нет. Персональные данные остаются на странице до копирования/отправки самим покупателем.
...
- Референсы и два изображения: https://chatgpt.com/share/6aa1a273-3bd8-83ed-9c35-7b4b0b5459e1.
- Изображения сохранены локально: работа сайта не зависит от временных ссылок на файлы в чате. Это визуализации, что отмечено на странице.
- На компьютере форма находится справа, пример слева. При ширине до 760 px форма идёт первой, примеры под ней. При ширине до 430 px контактные поля стоят в один столбец.
- Окно заказа прокручивается на небольших экранах; кнопки соцсетей при ширине до 600 px стоят в один столбец. Анимация появления учитывает системное уменьшение движения.
...
Доставка пока сохранена из первой демонстрационной версии: самовывоз 0 ₽, курьер по городу 900 ₽, по России 1 490 ₽. Эти тарифы и город самовывоза/курьерской доставки ещё требуют подтверждения владельца. Для стандартного размера итог равен 5 000 ₽ плюс доставка; для своего размера и стоимость зеркала, и итог уточняются в чате.
...
Эти проверки новой страницы и сборка прошли 09.09.2026. Локальная собранная страница вернула HTTP 200. Интерактивная проверка в браузере в эту переделку не входила.
...
При проверке сценария вручную: пустая форма не открывает окно; смена размера/доставки меняет сумму; адрес обязателен только для доставки; в тексте присутствуют все поля; копирование работает или даёт понятную инструкцию; закрытие/повторное оформление сохраняет актуальные данные; кнопки ведут на нужные аккаунты. Тестовые сообщения владельцу отправлять не нужно.
...
- Архив собирается штатным `scripts/package-site.sh` плагина Sites из готовой сборки; затем версия сохраняется и размещается с проверкой итогового статуса.
- При работе в Codex следовать актуальным навыкам `sites-building` и `sites-hosting`. Для Windows штатный скрипт запускается через Git Bash, пути передаются в формате Unix через `cygpath`.
