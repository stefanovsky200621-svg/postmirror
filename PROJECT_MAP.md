...
- Адрес вводится от большего к меньшему в трёх полях: обязательные `regionCity` («Регион и город») и `streetHouse` («Улица и дом»), затем необязательное `addressDetails` для квартиры, корпуса, подъезда и этажа. `handleSubmit` объединяет заполненные части через запятую в одну строку «Адрес:».
- Поле `regionCity` связано с локальным `datalist` из массива `citySuggestions`: при вводе города браузер показывает варианты, включая Балаково и Балашов. Функция `regionFirst` переставляет выбранный вариант в порядок «регион, город». Любой населённый пункт можно ввести без подсказки. Внешний адресный сервис не используется, поэтому улицы и дома автоматически не подбираются и введённый адрес никуда не отправляется до действий самого покупателя.
...
При проверке сценария вручную: пустая форма не открывает окно; смена размера/доставки меняет сумму; при самовывозе адрес скрыт и отсутствует в заявке; для курьера по СПб и доставки по России регион с городом, улица и дом обязательны; ввод «бала» предлагает Балаково и Балашов в порядке «Саратовская область, Балаково/Балашов»; части адреса объединяются в одну строку заявки от большего к меньшему; копирование работает или даёт понятную инструкцию; закрытие/повторное оформление сохраняет актуальные данные; кнопки ведут на нужные аккаунты. Тестовые сообщения владельцу отправлять не нужно.
